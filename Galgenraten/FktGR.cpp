...
//#include <Windows.h>	// nur f�r Sleep()

#include "FktDisp.h"	//Bildschirmanzeigen f�r Galgenraten
#include "FktGR.h"		//Funktionen f�r Galgenraten
...
struct TProtokoll neuGalgen(struct wort *Wortliste, struct TProtokoll *Pr, int Gk, int Nummer, int d) {
	...
	char tmpTaste[2] = {};				// Nutzereingabe als Zeichenkette f�r einfache Handhabung von strcat_s
	...
	struct wort *zWort;
	zWort = Wortliste;
	int znum = Zufall(anzWoerter(Wortliste)-1); // Zufallszahl zwischen 0 und Anzahl der W�rter in Liste (-1)

	for (int i=0; i<znum; i++)	{				// Zuf�lliges Wort aus der Wortliste w�hlen
		zWort=zWort->next;
	}
	strcpy(Suchwort, zWort->Zeichen);			// gew�hltes Wort nach Suchwort
		...
		&& ((fehlendeZeichen(charsGK(Suchwort,Gk), charsGK(Eingabe,Gk)) > 0 ) || (lenWort(Eingabe) == 0)))				// Nich alle Zeichen erraten, oder keine bisher Eingegeben
	{
		...
		if (tmpTaste[0] == 60) {
			...
			if (vorhandeneZeichen(Eingabe, tmpTaste) == 0) {	// falls Buchstabe tmpTaste noch nicht in Eingabe
				strcat_s(Eingabe, tmpTaste);					// anh�ngen an Eingabe
				hinzuTProtokoll(&Pr,Gk*Nummer,zWort->Zeichen,tmpTaste,d);	// TippProtokoll hinzuf�gen
			} 
		}
...
void anzeigenWoerter(struct WProtokoll *WPr, int d){
	// gibt alle bisher im Protokoll enthaltenen W�rter aus
	teilTitel(d);
	printUeberschrift("Anzeigen aller im Protokoll vorhandenen W\224erter");
	printWProtokoll(&WPr,d);		// Alle W�rter ausgeben
}


void anzeigenTippVerlauf(struct WProtokoll *WPr, int d){
	// l�sst den Anwender ein Wort aus dem WPr ausw�hlen und zeigt dann den Tippverlauf f�r dieses
	...
	while (tmpTaste != '\r'){				// bis mit Enter best�tigt
		teilTitel(d);
		printUeberschrift("Rateverlauf anzeigen");
		printf("\tWort f\201r Rateverlauf ausw\204hlen:\n\n");

		printWProtokollEinzelwoerter(&WPr, WortNummer, d);	// Liste der W�rter mit Marker auf WortNummer
		...
		if (tmpTaste == 72 && WortNummer > firstWortNummer) { // Pfeiltaste nach oben
			...
			while (isInWProtokoll(&WPr,WortNummer,d) == 0){
				WortNummer--;		// Index f�r das n�chste setzen (relevant element gel�scht)
			}
		...
		if (tmpTaste == 80 && WortNummer < lastWortNummer) { // unten
			WortNummer++;
			while (isInWProtokoll(&WPr,WortNummer,d) == 0){
				WortNummer++;		// Index f�r das vorherige setzen (relevant element gel�scht)
			}
...
struct WProtokoll loeschenTippVerlauf(struct WProtokoll *WPr, int d){
	// l�scht einen TippVerlauf, gibt Liste zur�ck
	...
	while (tmpTaste != '\r'){				// bis mit Enter best�tigt
		...
		if (tmpTaste == 72 && WortNummer > firstWortNummer) { // Pfeiltaste nach oben
			WortNummer--;
			while (isInWProtokoll(&WPr,WortNummer,d) == 0){
				WortNummer--;		// Index f�r das n�chste setzen (relevantes Element gel�scht/nicht vorhanden)
			}
			if(d)printf("Debug: nach oben\n"); 
		}		
		if (tmpTaste == 80 && WortNummer < lastWortNummer) { // unten
			WortNummer++;
			while (isInWProtokoll(&WPr,WortNummer,d) == 0){
				WortNummer++;		// Index f�r das vorherige setzen (relevantes Element gel�scht/nicht vorhanden)
			}
			if(d)printf("Debug: nach unten\n"); 
		}	
	}
	...
	if (WortNummer>0){
		...
		if (lauf->Nummer != 0) {
			*WPr = entferneWProtokollNummer(&WPr,WortNummer,d);		// Entfernt das gew�hlte Element
		}
	}else{
		printf("\n\tWort nicht gefunden.");
		Taste(d);
...
void abspielenTippVerlauf(struct TProtokoll *TPr, int d){
	...
	if (lauf->next != NULL){						// falls wort nicht leer
		if(lauf->Nummer<0){	Gk = 1; }				// Gro�Klein beachten?
		...
		while (lauf->next != NULL) {				// Solange weiterer Tipp im TippProtokoll
			...
			lauf = lauf->next;						// zum n�chsten buchstaben
			if (Taste(d) == '\r') {
				return;
			}
		}
...
struct wort eigeneWortliste(int d){
	// laden einer eigenen Wortliste, gibt diese zur�ck
	char Pfad[256] = {};
...
struct WProtokoll eigenesWProtokoll(int d){
	// laden eines eigenen Protokolls, gibt dieses zur�ck
	char Pfad[256] = {};
...
void eignesSpeicherzielWProtokoll(struct WProtokoll *WPr, int d){
	// Speichern eines WortProtokolls mit w�hlbarem Dateiname
	char Pfad[256] = {};
