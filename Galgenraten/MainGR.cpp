...
int main(){
	...
	// Initialisierung der verketteten Listen Wort, TProtokoll, WProtokoll
	...
	*WortProtokoll = lesenWProtokoll(NULL,d);
	...
	while (beenden != 1){	// Bleibe im Hauptmenu bis beenden gesetzt ist
		...
		case 'n':	if(d)printf("Debug: neues Spiel.\n");
			...
			while (WahlTaste == ' '){					// Schleife solange nach dem Spiel Leertaste gedrückt -> neues Spiel starten
				...
				*TippProtokoll = neuGalgen(Woerter, TippProtokoll, GrossKlein, (letztesWProtokoll(&WortProtokoll,d)).Nummer+1, d); // neues Spiel
				...
				WahlTaste = 0;
				while ((TippProtokoll->Nummer != 0) && (WahlTaste != '\r') && (WahlTaste != ' ')){ // wenn TippProtokoll nicht leer warte auf Tastendruck: Mit Enter zum Hauptmenu, Leertaste weiteres Wort
					WahlTaste = Taste(d);
				}
			}
		...
		case 'd':	if(d){d=0;}else{d=1;} if(d)printf("\n\nDebuginganzeige umgeschaltet\n\n"); break;	
	...
	if(WortProtokoll->Nummer != 0) eignesSpeicherzielWProtokoll(WortProtokoll,d); // mit Dateinamenswahl
